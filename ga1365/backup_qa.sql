...
FROM DBQNL20_2016_OWNER.rc_rman_status s
...
AND command_id like 'BACKUP%' AND s.status not in ('COMPLETED WITH WARNINGS','COMPLETED')
UNION ALL
SELECT to_char(s.start_Time,'dd-MON-YY hh24:mi:SS') start_time
      ,s.db_name
      ,s.status
   --  ,s.row_type
      ...
      ,s.output_device_Type
      ,ROUND( (s.end_time - s.start_time) * 24 * 60,2) duration
FROM DBQNL23_2016_OWNER.rc_rman_status s
 where trunc(start_time) >=(sysdate -2)
AND s.operation NOT IN ('RMAN'
                         ,'LIST'
                         ,'REPORT'
...
AND command_id like 'BACKUP_L%' AND s.status not in ('COMPLETED WITH WARNINGS','COMPLETED')
UNION ALL
SELECT to_char(s.start_Time,'dd-MON-YY hh24:mi:SS') start_time
      ,s.db_name
      ,s.status
   --  ,s.row_type
      ...
      ,s.output_device_Type
      ,ROUND( (s.end_time - s.start_time) * 24 * 60,2) duration
FROM DBQNL24_2016_OWNER.rc_rman_status s
--WHERE to_date(trunc(s.start_time),'DD-Mon-YYYY') >= to_date(trunc(sysdate - 7 + 8/24),'DD-Mon-YYYY')
 where trunc(start_time) >=(sysdate -2)
AND s.operation NOT IN ('RMAN'
                         ,'LIST'
...
SELECT to_char(s.start_Time,'dd-MON-YY hh24:mi:SS') start_time
   ...
   --  ,s.row_type
      ...
      ,s.output_device_Type
      ,ROUND( (s.end_time - s.start_time) * 24 * 60,2) duration
FROM DBQNL25_2016_OWNER.rc_rman_status s
--WHERE to_date(trunc(s.start_time),'DD-Mon-YYYY') >= to_date(trunc(sysdate - 7 + 8/24),'DD-Mon-YYYY')
 where trunc(start_time) >=(sysdate -2)
AND s.operation NOT IN ('RMAN'
                         ,'LIST'

...
COLUMN full_path              FORMAT a85                  HEAD 'ASM File Name / Volume Name / Device Name'
    ...
    ( SELECT
      ...
      FROM
          v$asm_file f RIGHT OUTER JOIN v$asm_alias     a USING (group_number, file_number)
                                   JOIN v$asm_diskgroup g USING (group_number) where upper(g.name) like upper('%&Diskgroup_Name%')
    ) db_files

...
WHERE
  ...
  AND owner NOT IN ('SYS','SYSTEM','SYSMAN','XDB','WMSYS','OLAPSYS','CTXSYS','DBSNMP','MDSYS','EXSYS') 
  AND partitioned = 'NO' 
UNION 
SELECT
    owner           owner
...
WHERE
      last_analyzed IS NULL 
  AND table_owner NOT IN ('SYS','SYSTEM','SYSMAN','XDB','WMSYS','OLAPSYS','CTXSYS','DBSNMP','MDSYS','EXSYS') 
UNION 
SELECT
    index_owner       owner
...
WHERE
      last_analyzed IS NULL 
  AND index_owner NOT IN ('SYS','SYSTEM','SYSMAN','XDB','WMSYS','OLAPSYS','CTXSYS','DBSNMP','MDSYS','EXSYS')
ORDER BY
    1
  , 2
  , 3

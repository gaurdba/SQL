...
{
        count=0
        for i in `cat $container`
        do
                if [ $count = 0 ]
                then
                        sqlplus -s "/ as sysdba" <<EOF >$sgadet
...
EOF
                ...
                sqlplus -s "/ as sysdba" <<EOF >>$sgadet
                set lines 300 pages 1 heading on feedback off term off
...
list_container()
{
	for db in `ps -ef|grep ora_pmon|grep -v grep|awk '{print $8}'|grep -v zdm|cut -d '_' -f3`
	do
		echo $db
		export ORACLE_SID=$db
		. oraenv >/dev/null
        sqlplus -s "/ as sysdba" <<EOF >$container
        col name for a20
        col profile for a35
...
sgadet=`mktemp -p /tmp sga.XXXXXXXXXX`
tsizepdb=0
tsizecdb=0
color_code
print_header
